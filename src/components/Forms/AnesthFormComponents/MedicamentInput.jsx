...
export const MedicamentInput = ({
  ...
  const handleSelectMedicament = (title) => {
    setInputValue(title);
    setFilteredMedicaments([]);

    const existsInMedicaments = medicaments.some(
      (med) => med.title.toLowerCase() === title.toLowerCase()
    );

    if (!existsInMedicaments) {
      updateMedicaments([...medicaments, { title }]);
      setShowModal(true);
      setInputValue("");
    }
  };
    ...
    <>
      ...
      >
        {filteredMedicaments.map((med) => (
          <li key={med.id} onClick={() => handleSelectMedicament(med.title)}>
            {med.title}
          </li>
        ))}
      </ul>
